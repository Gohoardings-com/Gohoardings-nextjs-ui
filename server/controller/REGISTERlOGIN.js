        ...
        const {email,name, image } = session.user
          const  selectResult  = await executeQuery("SELECT * FROM tblcontacts WHERE email='" + email + "'","gohoardi_crmapp",next)
                 ...
                 const result = await executeQuery("SELECT userid From tblcontacts ORDER By userid DESC LIMIT 1","gohoardi_crmapp",next)
                   ...
                   const userData = await executeQuery(`Insert into tblcontacts (firstname, email, password, userid, profile_image, provider) VALUES ('${name}','${email}','${password}','${userid}','${image}','linkdin')`, "gohoardi_crmapp",next)
                                    if (userData) {
...
exports.companyDetails =  catchError(async (req,res, next) =>{
    ...
    let sql;
    if(!pan){    
        sql =await executeQuery ("UPDATE  tblclients SET  company='"+company+"', phonenumber='"+phone+"', city='"+city+"', zip='"+zip_code+"',state='"+state+"', address='"+address+"', website='"+website+"',  gstin='"+gstin+"' WHERE userid=" + userId + " ", "gohoardi_crmapp",next)
    }else{
        sql = await executeQuery("UPDATE  tblclients SET  company='"+company+"', phonenumber='"+phone+"', city='"+city+"', zip='"+zip_code+"',state='"+state+"', address='"+address+"', website='"+website+"', pan='"+pan+"', gstin='"+gstin+"' WHERE userid=" + userId + " ", "gohoardi_crmapp",next)
    }
        if (!sql) {
            ...
            return jwtToken.verify(token, "thisismysecretejsonWebToken", async (err, user) => {
                if (err) {
                    ...
                    if (password == confirmPassword) {
                        ...
                        const sql =await executeQuery ("UPDATE tblcontacts SET password ='" + finalPassword + "' WHERE id='" + userId + "'","gohoardi_crmapp", next );
                            if (sql) {  
